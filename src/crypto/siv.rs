...
impl AeadDecryptor for MiscreantCipher {
    fn decrypt(&mut self, input: &[u8], output: &mut [u8]) -> CipherResult<()> {
        ...
        let result = match self.cipher {
            MiscreantCryptoVariant::Aes128(ref mut cipher) => cipher.decrypt_in_place(&self.nonce, b"", &mut buf),
            MiscreantCryptoVariant::Aes256(ref mut cipher) => cipher.decrypt_in_place(&self.nonce, b"", &mut buf),
        };
