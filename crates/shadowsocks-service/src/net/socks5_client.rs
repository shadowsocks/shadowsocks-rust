//! SOCKS5 TCP client for outbound proxy connections
...
use crate::config::{OutboundProxy, OutboundProxyAuth};
...
impl Socks5TcpClient {
    ...
    /// Connects to `addr` via a SOCKS5 proxy chain
    pub async fn connect_chain<A>(addr: A, proxies: &[OutboundProxy]) -> Result<Self, Error>
    where
        A: Into<Address>,
    {
        let mut proxies = proxies.iter();
        let Some(first_proxy) = proxies.next() else {
            return Err(io::Error::new(io::ErrorKind::InvalidInput, "empty outbound proxy chain").into());
        };

        let mut s = TcpStream::connect((first_proxy.host.as_str(), first_proxy.port)).await?;
        socks5_handshake(&mut s, first_proxy.auth.as_ref()).await?;

        for proxy in proxies {
            socks5_command(&mut s, Command::TcpConnect, proxy.address()).await?;
            socks5_handshake(&mut s, proxy.auth.as_ref()).await?;
        }
